<?php
/**
 * Campaign: BryanFFelt73473
 * Created: 2022-02-22 17:23:21 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rojeau5e7i';

$campaignSignature = 'yuNR0fj10QruzuGRwBwjo4j9W6gmo8JFdntnINgJ6u7PnMJcp3';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rojeau5e7i', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rojeau5e7i'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rojeau5e7i');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: yuNR0fj10QruzuGRwBwjo4j9W6gmo8JFdntnINgJ6u7PnMJcp3';
...
{
    return array_key_exists('16rojeau5e7i', $_COOKIE) ? $_COOKIE['16rojeau5e7i'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rojeau5e7i', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rojeau5e7i.log';
