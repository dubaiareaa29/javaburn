<?php
/**
 * Campaign: RobertCRobert644
 * Created: 2022-02-22 17:19:57 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rn4cq5fd4i';

$campaignSignature = '0fzs5nYhNAGiPr9Ashyj1v3N7Uj0WLVcolABiY4AYFJBSwu9K7';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rn4cq5fd4i', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rn4cq5fd4i'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rn4cq5fd4i');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: 0fzs5nYhNAGiPr9Ashyj1v3N7Uj0WLVcolABiY4AYFJBSwu9K7';
...
{
    return array_key_exists('16rn4cq5fd4i', $_COOKIE) ? $_COOKIE['16rn4cq5fd4i'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rn4cq5fd4i', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rn4cq5fd4i.log';
