<?php
/**
 * Campaign: DebbieDGagne6262
 * Created: 2022-02-21 21:10:26 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rchix00500';

$campaignSignature = 'Ov8rc56VEv2aBjmvV6lqXkpg1R0o1gjwntoAhTK6yySgvrMGmt';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rchix00500', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rchix00500'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rchix00500');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: Ov8rc56VEv2aBjmvV6lqXkpg1R0o1gjwntoAhTK6yySgvrMGmt';
...
{
    return array_key_exists('16rchix00500', $_COOKIE) ? $_COOKIE['16rchix00500'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rchix00500', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rchix00500.log';
