<?php
/**
 * Campaign: WandaRGregoire626
 * Created: 2022-02-21 21:14:41 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rcuab66n9r';

$campaignSignature = 'Vep17wqCs0vcsxdkqjG9KQ9EecJ5ln5wt5seVUmSrQgcDkp5U7';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rcuab66n9r', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rcuab66n9r'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rcuab66n9r');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: Vep17wqCs0vcsxdkqjG9KQ9EecJ5ln5wt5seVUmSrQgcDkp5U7';
...
{
    return array_key_exists('16rcuab66n9r', $_COOKIE) ? $_COOKIE['16rcuab66n9r'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rcuab66n9r', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rcuab66n9r.log';
