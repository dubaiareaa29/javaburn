<?php
/**
 * Campaign: GeorgeAGarcia6262
 * Created: 2022-02-21 21:27:24 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rd71pcd5ji';

$campaignSignature = 'WNzUyZJDoudMaOilTDYm1qRlCxc3dIF24VnPx3Bfmoyl2drDWQ';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rd71pcd5ji', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rd71pcd5ji'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rd71pcd5ji');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: WNzUyZJDoudMaOilTDYm1qRlCxc3dIF24VnPx3Bfmoyl2drDWQ';
...
{
    return array_key_exists('16rd71pcd5ji', $_COOKIE) ? $_COOKIE['16rd71pcd5ji'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rd71pcd5ji', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rd71pcd5ji.log';
