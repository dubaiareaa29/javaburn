...
$campaignId = '16rkzsd4cbi0';

$campaignSignature = 'WUhHxYQptsTWrwpp0K4Yk80naGlMfchurHPLBFoSj0S2EP2aIW';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rkzsd4cbi0', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rkzsd4cbi0'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rkzsd4cbi0');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: WUhHxYQptsTWrwpp0K4Yk80naGlMfchurHPLBFoSj0S2EP2aIW';
...
{
    return array_key_exists('16rkzsd4cbi0', $_COOKIE) ? $_COOKIE['16rkzsd4cbi0'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rkzsd4cbi0', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rkzsd4cbi0.log';
