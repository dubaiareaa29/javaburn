<?php
/**
 * Campaign: StephanieSCollins454
 * Created: 2022-02-22 17:20:46 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rnh44blve9';

$campaignSignature = 'aXhiZw9QtI99RKI3zy5x9vaNteTny7WHoNS99JJoZVnddCDO7Y';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rnh44blve9', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rnh44blve9'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rnh44blve9');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: aXhiZw9QtI99RKI3zy5x9vaNteTny7WHoNS99JJoZVnddCDO7Y';
...
{
    return array_key_exists('16rnh44blve9', $_COOKIE) ? $_COOKIE['16rnh44blve9'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rnh44blve9', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rnh44blve9.log';
