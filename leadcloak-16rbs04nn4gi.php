<?php
/**
 * Campaign: GloriaDMiddleton522
 * Created: 2022-02-21 21:01:07 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rbs04nn4gi';

$campaignSignature = 'M0kHGaStRGYnw06OfRT8I7zI0VwxUpSSlqpO6NZ2pIj7mj3F2l';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rbs04nn4gi', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rbs04nn4gi'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rbs04nn4gi');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: M0kHGaStRGYnw06OfRT8I7zI0VwxUpSSlqpO6NZ2pIj7mj3F2l';
...
{
    return array_key_exists('16rbs04nn4gi', $_COOKIE) ? $_COOKIE['16rbs04nn4gi'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rbs04nn4gi', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rbs04nn4gi.log';
