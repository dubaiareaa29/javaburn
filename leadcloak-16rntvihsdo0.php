<?php
/**
 * Campaign: ReginaldSCosta6363
 * Created: 2022-02-22 17:21:38 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rntvihsdo0';

$campaignSignature = 'menAvW7QU6rW9dBBQHWcf6Eutz3IipFWFyN17dOm6vTuDIBfTW';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rntvihsdo0', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rntvihsdo0'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rntvihsdo0');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: menAvW7QU6rW9dBBQHWcf6Eutz3IipFWFyN17dOm6vTuDIBfTW';
...
{
    return array_key_exists('16rntvihsdo0', $_COOKIE) ? $_COOKIE['16rntvihsdo0'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rntvihsdo0', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rntvihsdo0.log';
