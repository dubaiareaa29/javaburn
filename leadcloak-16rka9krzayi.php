...
$campaignId = '16rka9krzayi';

$campaignSignature = 'qGVXBsGhLxE08O72eAzPTxMvTD5hDrD9GxUy5PZZwmjnZftRB0';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rka9krzayi', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rka9krzayi'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rka9krzayi');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: qGVXBsGhLxE08O72eAzPTxMvTD5hDrD9GxUy5PZZwmjnZftRB0';
...
{
    return array_key_exists('16rka9krzayi', $_COOKIE) ? $_COOKIE['16rka9krzayi'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rka9krzayi', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rka9krzayi.log';
