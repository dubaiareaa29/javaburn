...
$campaignId = '16rbf8qhgm6r';

$campaignSignature = 'uNosRXOdtVMj5y0Gbx2sY69hFjkeHBURqN03qL5N34OVzKhP3V';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rbf8qhgm6r', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rbf8qhgm6r'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rbf8qhgm6r');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: uNosRXOdtVMj5y0Gbx2sY69hFjkeHBURqN03qL5N34OVzKhP3V';
...
{
    return array_key_exists('16rbf8qhgm6r', $_COOKIE) ? $_COOKIE['16rbf8qhgm6r'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rbf8qhgm6r', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rbf8qhgm6r.log';
