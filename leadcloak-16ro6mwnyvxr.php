<?php
/**
 * Campaign: PatriciaIRios36346
 * Created: 2022-02-22 17:22:23 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16ro6mwnyvxr';

$campaignSignature = 'OYyWRSL2T32KQVKSuJ5m7Kes1RfFn0qkqgpDCQqGZPmvwuV7Hi';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16ro6mwnyvxr', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16ro6mwnyvxr'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16ro6mwnyvxr');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: OYyWRSL2T32KQVKSuJ5m7Kes1RfFn0qkqgpDCQqGZPmvwuV7Hi';
...
{
    return array_key_exists('16ro6mwnyvxr', $_COOKIE) ? $_COOKIE['16ro6mwnyvxr'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16ro6mwnyvxr', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16ro6mwnyvxr.log';
