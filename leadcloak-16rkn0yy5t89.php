<?php
/**
 * Campaign: RebeccaJSullivan3533
 * Created: 2022-02-22 11:30:17 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rkn0yy5t89';

$campaignSignature = 'TMz7FCEH8fnzneMddt8FrPOwYUwkUNAUC0qqkpVkwDo1P0GBdS';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rkn0yy5t89', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rkn0yy5t89'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rkn0yy5t89');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: TMz7FCEH8fnzneMddt8FrPOwYUwkUNAUC0qqkpVkwDo1P0GBdS';
...
{
    return array_key_exists('16rkn0yy5t89', $_COOKIE) ? $_COOKIE['16rkn0yy5t89'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rkn0yy5t89', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rkn0yy5t89.log';
