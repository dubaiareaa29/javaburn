<?php
/**
 * Campaign: DavidCRider335
 * Created: 2022-02-23 15:39:18 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rtut7ev09r';

$campaignSignature = '2BiNnQBNdQ4J7nCBHlAShPrEMAfuBZP8dQSMUxOm78zZ167oEa';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rtut7ev09r', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rtut7ev09r'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rtut7ev09r');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: 2BiNnQBNdQ4J7nCBHlAShPrEMAfuBZP8dQSMUxOm78zZ167oEa';
...
{
    return array_key_exists('16rtut7ev09r', $_COOKIE) ? $_COOKIE['16rtut7ev09r'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rtut7ev09r', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rtut7ev09r.log';
