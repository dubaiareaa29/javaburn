<?php
/**
 * Campaign: SarahRRobinson744
 * Created: 2022-02-22 17:24:04 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16row5p0bwh9';

$campaignSignature = 'vmO7jtlUw4jjzpIFMd2DK9GXlbepXXw0olFIWZl4kBGBW0PALI';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16row5p0bwh9', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16row5p0bwh9'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16row5p0bwh9');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: vmO7jtlUw4jjzpIFMd2DK9GXlbepXXw0olFIWZl4kBGBW0PALI';
...
{
    return array_key_exists('16row5p0bwh9', $_COOKIE) ? $_COOKIE['16row5p0bwh9'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16row5p0bwh9', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16row5p0bwh9.log';
