...
$campaignId = '16rmetxt2cl0';

$campaignSignature = '2Aa5tqX9grjVfcbpZLsyaW1E7LTZNH4oq7IV6ONlPMuPaTCSK6';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rmetxt2cl0', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rmetxt2cl0'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rmetxt2cl0');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: 2Aa5tqX9grjVfcbpZLsyaW1E7LTZNH4oq7IV6ONlPMuPaTCSK6';
...
{
    return array_key_exists('16rmetxt2cl0', $_COOKIE) ? $_COOKIE['16rmetxt2cl0'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rmetxt2cl0', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rmetxt2cl0.log';
