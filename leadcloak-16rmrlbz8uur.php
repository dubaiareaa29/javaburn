<?php
/**
 * Campaign: GrantCLittle555
 * Created: 2022-02-22 17:19:03 UTC
 */

// ---------------------------------------------------
// Configuration

$campaignId = '16rmrlbz8uur';

$campaignSignature = 'JzEyfTHBKJKAYdHdJLXd27dcrBokbVZPUzizZ0BxOgYES5kjOv';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rmrlbz8uur', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rmrlbz8uur'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rmrlbz8uur');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: JzEyfTHBKJKAYdHdJLXd27dcrBokbVZPUzizZ0BxOgYES5kjOv';
...
{
    return array_key_exists('16rmrlbz8uur', $_COOKIE) ? $_COOKIE['16rmrlbz8uur'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rmrlbz8uur', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rmrlbz8uur.log';
