...
$campaignId = '16rjkqsfmaf0';

$campaignSignature = 'ZDBrxVAGtvJensb5ajR7o9WldeQvAW7toHBW2x2wIwusJSNI6l';
...
function httpHandleResponse($response, $logToFile = true)
{
	...
	if (is_array($decodedResponse) && array_key_exists('error', $decodedResponse)) {
        ...
        file_put_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rjkqsfmaf0', $response);
...
{
    if (httpTaggedUser() && file_exists(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rjkqsfmaf0'))
    {
        return file_get_contents(sys_get_temp_dir().DIRECTORY_SEPARATOR.'16rjkqsfmaf0');
    }
...
function httpGetAllHeaders()
{
	...
	$headers[] = 'X-LC-SIG: ZDBrxVAGtvJensb5ajR7o9WldeQvAW7toHBW2x2wIwusJSNI6l';
...
{
    return array_key_exists('16rjkqsfmaf0', $_COOKIE) ? $_COOKIE['16rjkqsfmaf0'] : 0 ;
}

function httpTagUser($type, $life)
{
    setcookie('16rjkqsfmaf0', $type, time() + $life * 60 * 60 * 24, '/');
}
...
function logToFile($result)  {
	...
	$filename = 'leadcloak-log-16rjkqsfmaf0.log';
